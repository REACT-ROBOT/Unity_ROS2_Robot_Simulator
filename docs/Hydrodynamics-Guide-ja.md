...
浮力は排水した水の重さ (アルキメデスの原理) です:
`F = ρ水 × V排水 × g` に、コライダの水没率を掛けたもの。
全没時の排水体積は次の優先順で決まります:

1. **体積の明示** — `buoyancy_material` 内 (または `collision` 側の参照内) の
   `<volume value="0.12"/>` で排水体積 [m³] を直接指定。材料体積より排水量が
   ずっと大きい中空船体にはこれを使います。
2. **`<volume value="auto"/>`** — コリジョン形状から体積を計算
   (box/sphere/capsule コライダは厳密、閉じたメッシュはメッシュ体積)。
   浮くかどうかは `<inertial>` の実質量と排水質量の比較で決まり、現実と同じです。
3. **既定 (density のみ)** — `V = mリンク / (1000 × density)`、つまりリンクを
   その比重の均質な物体として扱います。上記の浮沈規則はリンク質量に依らず成り立ち、
   `density 0.5` の物体は半分沈んで浮きます。**リンクの実質量** (`<inertial>`) を
   使います。

力はリンク自身の ArticulationBody に加わります。水機能を宣言しても、コリジョン
ジオメトリに見えない 1 kg のボディが付くことはもうありません (以前はコリジョン
1 個につき約 1 kg 増え、接地反力も歪んでいました)。
