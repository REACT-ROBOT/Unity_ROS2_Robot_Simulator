...
| SDF worlds beyond the supported subset | SDF `.world` models load as static scenery: non-static models do not fall or push, `<joint>`s do not move, and `<population>`/`<physics>`/`<plugin>` are dropped (reported in `error_message`). `<actor>` is supported only as a shape following a `<trajectory>` (no skeletal animation). Dynamic robots belong to entities spawned from URDF; letting a world bring robots with it is the separate deferred "world includes entities" idea | [Simulation-Interfaces-Services.md](Simulation-Interfaces-Services.md), SDF worlds |
