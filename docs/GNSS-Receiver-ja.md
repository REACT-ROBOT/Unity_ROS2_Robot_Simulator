...
エンティティパネルの `<link> gnss rays` で、**衛星ごとの信号がどう届いたか**を描けます。
`SIM_AUTO_SENSOR_VIZ=1` で起動すれば最初のエンティティで自動的に有効になります。

| 色 | 意味 |
|---|---|
| 緑（細） | 直達。使える信号 |
| **橙（太）** | **反射で届いた経路 (NLOS)。アンテナ → 壁 → 空へと折れ曲がる** |
| 赤（短い・薄い） | 遮蔽されて届かない |

橙だけ太くしてあります。20 本の直達に対して解を動かしているのは跳ね返った 2〜3 本であり、
画の中で見るべきものはそれだからです。折れ曲がりそのものが、受信機が距離として
測ってしまう遠回り（過剰行路長）です。

**深いキャニオンは横から中が見えません。** 幅 10 m・高さ 40 m のスロットを覗くには
ほぼ真上からになります。撮影用には浅めの壁（例: 幅 16 m・高さ 14 m）にして、
街路の軸方向から撮ると経路がよく見えます。
