...
| `stiffness` | 歯面係合時の伝達剛性 K | N·m/rad | 20 |
| `damping` | 係合時の伝達ダンピング D | N·m/(rad/s) | 0.5 |

`stiffness` の目安は 2 つの条件に挟まれます (詳細と実測値は
[検証レポートの「伝達剛性の選び方」](Servo-Model-Validation-ja.md)):

- **下限**: 歯面たわみ τ/K がガタ半幅 b より小さいこと (`K > τ/b`)。これを下回ると
  ガタではなく伝達のたわみが偏差を支配し、バックラッシとして観測できません
- **上限**: 不感帯の引数がステップ終端の負荷位置に対して 1 ステップ古いため、
  **K·ω·dt** のトルク誤差が乗ります。分解したいトルクに対してこれが十分小さいこと。
  Δt = 0.02 s なら、関節が ω = 0.15 rad/s で動く場面で 0.2 N·m を扱うなら K ≲ 50 が目安

既定値の 20 は、ガタが支配的なまま上限にも収まる範囲を狙った値です。実機に合わせて
明示指定するのが望ましく、付属の servo_demo は K = 2 を使用しています。

なお**ランタイム (ビルド済みアプリ) でスポーンしたロボットでは離散安定条件
...
- **伝達剛性の上限は 2 段階**: (1) 離散安定限界 — 回転子と負荷が物理1ステップ
  遅れで結合されるため、結合固有振動数 √(K·(1/Jm+1/J_load)) × Δt が ~1 を
  超えると自励発振します (50 Hz・軽負荷の実測で K ≈ 200〜250)。素の xDrive
  位置サーボ (servo_model なし) も同じ制限を受けます。(2) **定量精度の限界** —
  不感帯の引数が 1 ステップ古いため K·sech²(Δ/b)·ω·dt のトルク誤差が残り、
  こちらが先に効きます。50 Hz で ω = 0.15 rad/s・0.2 N·m を扱う場面なら
  K ≲ 50 が目安 (実測: K = 100 から歯面の判定が反転)
...
- **`ArticulationBody.jointVelocity` は環境を問わず使えません**: xDrive が
  駆動している関節では、その運動をまったく反映しません (素のドライブで
  0.15 rad/s 掃引中の報告値は −0.008 rad/s、静止保持中は約 0.08 rad/s の
  ゴミが乗る)。ドライブのない自由関節では正確なので、自由落下で確かめると
  見逃します。モデルは負荷速度を位置の有限差分から求めます。**`/joint_states`
  の velocity もこの値なので、制御側で使う場合は注意してください**
